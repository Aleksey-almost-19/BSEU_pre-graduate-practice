...
async function showLoanDetails(loanId) {
    ...
    try {
        ...
        modalContent.innerHTML = `
            ...
            <!-- КНОПКА ОФОРМИТЬ ЗАЯВКУ (ИСПРАВЛЕННАЯ) -->
            <button class="tg-button" onclick="showApplicationForm(${loanId})">
                📝 Оформить заявку
            </button>
...
function showApplicationForm(loanId) {
    ...
    const formHtml = `
        <div style="padding: 10px;">
            <h3 style="color: #4caf50; margin-bottom: 20px; text-align: center;">Оформление заявки</h3>
            
            <div style="margin-bottom: 15px;">
                <label style="display: block; margin-bottom: 5px; color: #ccc;">Ваше ФИО</label>
            ...
            <div style="margin-bottom: 20px;">
                <label style="display: block; margin-bottom: 5px; color: #ccc;">Номер телефона</label>
            ...
            <div style="display: flex; gap: 10px;">
                <button onclick="submitApplication(${loanId})" 
                    style="flex: 2; background: #4caf50; color: white; border: none; padding: 14px; border-radius: 8px; font-weight: 600; cursor: pointer;">
                    ✅ Отправить заявку
                </button>
                <button onclick="closeFormModal()" 
                    style="flex: 1; background: #333; color: white; border: none; padding: 14px; border-radius: 8px; cursor: pointer;">
                    ❌ Отмена
                </button>
            </div>
        </div>
...
async function submitApplication(loanId) {
    ...
    // Простейшая проверка телефона (можно улучшить)
    if (phone.length < 5) {
...
// Обновленная функция applyForLoan (теперь открывает форму)
function applyForLoan(loanId) {
    showApplicationForm(loanId);
}
